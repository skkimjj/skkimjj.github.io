...
3. 네이버 버전(첫 줄 `제목: ...`, 【】 소제목, 해시태그, 면책 문구) → `briefing-work/naver.md`
   - **표는 마크다운 표 그대로 유지한다** (원 사양의 '항목: 값' 줄글 변환 규칙은 폐기 —
     웹페이지에서 네이버 편집기로 복붙 시 표가 그대로 옮겨진다)
   - **줄 간격**: 모든 【소제목】 앞, 그리고 마지막 해시태그 줄의 앞뒤에
     `&nbsp;` 한 줄짜리 단락을 넣는다 (네이버는 CSS 여백을 무시하므로 실제 빈 줄 필요)

**하이라이트 규칙 (두 버전 공통):**
- 독자가 놓치면 안 되는 핵심 문구·수치에 `<mark>...</mark>` 태그를 감싼다.
  예: `<mark>SOX -4.78% 급락</mark>`, `<mark>오늘 밤 22:30 미국 CPI</mark>`
- 과하지 않게: 섹션(소제목)당 최대 1~2개, 글 전체 5~8개 수준.
- **괴리율 트래커 표의 셀 안에는 넣지 않는다** (검증 파서가 숫자를 읽는 곳).
