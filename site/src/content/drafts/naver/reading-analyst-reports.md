...
▶ **예측오차 절댓값 45.22%** = 목표주가와 실제 주가가 평균 45% 떨어져 있었다는 뜻입니다. '절댓값'이 중요한데, 목표주가보다 덜 오른 건과 더 오른 건을 그냥 평균하면 서로 지워져 오차가 작아 보이기 때문에 **부호를 떼고 벌어진 거리만 평균한** 값입니다. 그래서 평균끼리의 차이(34% vs 4%)보다 큽니다 — 리포트를 한 건씩 놓고 보면 어긋난 폭이 그만큼 크다는 뜻이죠.
