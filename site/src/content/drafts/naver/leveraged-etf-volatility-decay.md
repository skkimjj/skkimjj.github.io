...
▶ **① 방향이 정반대인 두 상품이 똑같이 깎였습니다.** +2배는 기초자산이 오르면 버는 상품이고, −1배는 내리면 버는 상품입니다. 정반대에 걸어 둔 셈인데 열흘 뒤 결과가 −8.77%로 똑같습니다.
...
101%는 75%의 약 1.35배인데, 1.35를 두 번 곱하면 약 1.8이니 깎이는 몫은 <mark>약 1.8배</mark>가 됩니다. **같은 2배 배율, 같은 보유기간이라도 단일종목 쪽 잠식이 구조적으로 크다**는 뜻이죠. 다만 자본시장연구원은 이 변동성 상승을 단일종목 ETF 탓으로 단정하지 않았습니다. 같은 기간 마이크론(85%→126%) 등 해외 반도체 종목의 변동성도 함께 올랐고, 거시환경 불확실성 등 여러 요인이 겹친 결과로 봤습니다.
